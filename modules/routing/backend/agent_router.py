...
def _resolve_framework_tools_dir() -> Path:
    for parent in Path(__file__).resolve().parents:
        for candidate in (parent / 'backend' / '.bago' / 'tools', parent / '.bago' / 'tools'):
            if (candidate / '_path_helper.py').is_file():
                return candidate
    raise RuntimeError('Cannot locate BAGO framework tools directory')


FRAMEWORK_TOOLS_DIR = _resolve_framework_tools_dir()
if str(FRAMEWORK_TOOLS_DIR) not in sys.path:
    sys.path.insert(0, str(FRAMEWORK_TOOLS_DIR))
...
FRAMEWORK_BAGO_ROOT = FRAMEWORK_TOOLS_DIR.parent
...
def route_task(task: str, agents: list[dict] | None = None, use_classifier: bool = True, record: bool = False) -> dict:
    ...
    if os.environ.get('BAGO_ORCHESTRATE') == '1':
        ...
        _orc_path = FRAMEWORK_TOOLS_DIR / 'orchestrator_v4.py'
